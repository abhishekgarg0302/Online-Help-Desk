...
if(isset($_POST['submit'])){
 ...
 $query = mysqli_query($conn, "SELECT *  FROM user_registration WHERE pass='$pass' AND username=TRIM('$username') AND userid=TRIM('$cid') AND usertype=1");
...
// <h1>echo $rows</h1>
 if($rows == 1){
    ...
    $_SESSION['userid']= $cid;
    $name=mysqli_query($conn, "SELECT fullname FROM user_registration WHERE pass='$pass' AND username=TRIM('$username') AND userid=TRIM('$cid') AND usertype=1");
